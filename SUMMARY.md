# Summary

## IBM Cloud Sign-up

- [IBM Cloud Sign-up](http://ibm.biz/create-ibm-cloud)

## IBM Event Streams

- [IBM Event Streams](https://ibm.github.io/event-streams)
- Pre-requisite to Workshop
    - [Create Event Streams Service in IBM Cloud](ibm-event-streams/ibm-event-streams.md)
    - [Clone sample code from GitHub repository](https://github.com/IraAngeles-IBM/event-streams-samples)

## IBM Event Streams for Node.js 

- [Local setup](kafka-nodejs-console-sample/docs/Local.md)
- [Local Docker](kafka-nodejs-console-sample/docs/Docker_Local.md)
- [Cloud Foundry](kafka-nodejs-console-sample/docs/Cloud_Foundry.md)
    - [Classic Plan Deployment Guide](kafka-nodejs-console-sample//docs/CF_Classic_Plan.md)
    - [Standard/Enterprise Plan Deployment Guide](kafka-nodejs-console-sample/docs/CF_Standard_Enterprise_Plan.md)

- [Kubernetes Service](kafka-nodejs-console-sample/docs/Kubernetes_Service.md)


## IBM Event Streams for Python

- [Local setup](kafka-python-console-sample/docs/Local.md)
- [Local Docker](kafka-python-console-sample/docs/Docker_Local.md)
- [Cloud Foundry](kafka-python-console-sample/docs/Cloud_Foundry.md)
    - [Classic Plan Deployment Guide](kafka-python-console-sample/docs/CF_Classic_Plan.md)
    - [Standard/Enterprise Plan Deployment Guide](kafka-python-console-sample/docs/CF_Standard_Enterprise_Plan.md)

- [Kubernetes Service](kafka-python-console-sample/docs/Kubernetes_Service.md)

## Other Examples and Tutorials

- [kafka-connect](kafka-connect/README.md)
- [kafka-mirrormaker](kafka-mirrormaker/README.md)
- [Build librdkafka](libs/librdkafka.md)
- [Apache Kafka](https://developer.ibm.com/articles/an-introduction-to-apache-kafka/)
- [IBM Messaging Github Repo](https://github.com/ibm-messaging)

